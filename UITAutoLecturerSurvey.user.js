...
function UITAutoLecturerSurveyHomePage() {
  const headElement = document
    .getElementsByClassName('content')[0]
    .getElementsByTagName('center');
  const para = document.createElement('p');
  const node = document.createTextNode(
    '⭐ Bạn đang sử dụng UIT - Auto Lecture Survey bởi Kevin Nitro 💖'
  );
  para.appendChild(node);
  headElement[0].appendChild(para);
}
...
(function () {
  ...
  if (window.location.pathname === '/sinhvien/phieukhaosat') {
    UITAutoLecturerSurveyHomePage();
  } else UITAutoLecturerSurveyRunScript();
})();
